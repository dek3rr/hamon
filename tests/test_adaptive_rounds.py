...
def _setup(L, coupling, n_chains, *, key=0):
    ...
    nodes, edges, fb, ebms, progs = make_ising_grid(L, [float(b) for b in betas], coupling=coupling)
    inits = [hinton_init(k, ebms[-1], fb, ()) for k in jax.random.split(jax.random.key(key), n_chains)]
...
def test_legacy_mode_unchanged():
    ...
    stats = _run(ebm, prog, betas, inits, adaptive_tuning=False, n_tune=3, rounds_per_tune=20)
...
def test_adaptive_respects_round_caps():
    ...
    ebm, prog, fb, betas, inits = _setup(4, 0.5, 6)
    stats = _run(ebm, prog, betas, inits, n_tune=4, rounds_per_tune=200,
                 round_batch=50, min_rounds_per_tune=50)
...
def test_min_tune_phases_floor():
    ...
    stats = _run(ebm, prog, betas, inits, n_tune=8, min_tune_phases=3, rounds_per_tune=100)
...
def test_adaptive_stops_early_and_saves_rounds():
    ...
    stats = _run(ebm, prog, betas, inits, n_tune=n_tune, rounds_per_tune=rounds_per_tune)
...
def test_compile_count_bounded():
    ...
    nrpt_adaptive(
        jax.random.key(3), ebm=ebm, program=prog, init_states=inits, clamp_state=[],
        n_rounds=40, gibbs_steps_per_round=1, initial_betas=betas, device="cpu",
        n_tune=3, rounds_per_tune=40, round_batch=40, min_rounds_per_tune=40,
    )
...
def test_discover_forwards_tune_tol():
    ...
    nodes, edges, fb, ebms, progs = make_ising_grid(4, [float(b) for b in betas], coupling=0.4)

    def init_factory(n_chains, chain_ebms, chain_programs):
        f = chain_programs[0].gibbs_spec.free_blocks
        return [hinton_init(k, chain_ebms[0], f, ()) for k in jax.random.split(jax.random.key(7), n_chains)]

    out = discover_chain_count(
        jax.random.key(1), ebm=ebms[-1], program=progs[-1], init_factory=init_factory,
        beta_range=(0.0, 1.0), gibbs_steps_per_round=1, initial_n=8,
        rounds_per_probe=80, n_tune_per_probe=3, max_iters=3, min_chains=3, max_chains=24,
        tune_tol=0.1, device="cpu",
    )
    assert 3 <= out["n_chains"] <= 24
    assert out["converged_reason"] in {"chain_count", "lambda_stable", "no_progress", "max_iters"}
